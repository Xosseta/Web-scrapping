...
def get_company_name():
    for link in links_sorted_list:
        company_name_link = requests.get(link, headers=get_headers())
        company_name_parsed = BeautifulSoup(company_name_link.text, 'lxml')
        company_name = company_name_parsed.find('span', {'data-qa': "bloko-header-2"})
        if not company_name:
            continue
        company_name_text = company_name.text
        company_name_normalized = unicodedata.normalize('NFKD', company_name_text)
